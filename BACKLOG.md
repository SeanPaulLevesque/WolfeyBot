...
-~~Scrappy~~ **DONE (0.38.0):** the Ghost component of the type product is neutralised (×1) in full_damage_calc for a Scrappy attacker's Normal/Fighting moves — only that component, so Close Combat vs Gengar is ×0.5 via Poison, not ×1. Flows through both directions (outgoing + incoming-threat facts) automatically; a Scrappy Lopunny-Mega now threatens our Ghosts (reviewed snapshot cells: Aegislash King's-Shields the HJK it previously ignored). Scrappy was already in INTIMIDATE_IMMUNE_ABILITIES. (Still open, and unmodeled generally: we don't apply an Intimidate -1 Atk prediction anywhere, so the ignores-Intimidate half has nothing to gate yet.)
