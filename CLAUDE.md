...
| 6 | ProtectValue | **Single row** on Protect-family moves when `ctx.is_threatened(slot)`: ×3.0 (2.5 → 5.0 in 0.45.0 to survive the unconditional LoneProtect ×0.5, then re-tuned to 3.0 in 0.45.2 after observed over-protecting — a threatened Protect nets 3.0 alone, 1.5 beside an attacker). The partner-clears ×3.0 is a phase-2 adjuster (`PartnerClearsAdjuster`); the 1v1/2v1 "Protect only delays" cancels are a separate `EndgameStallModule` (#7) — one concern per module |
| 7 | EndgameStall | Protect ×0.1 when `ctx.is_threatened(slot)` and the board is a 1v1 endgame or 2v1 advantage (Protect only delays). Net with ProtectValue ×3.0 is 0.3 — dropped from the 0.44.1 user-tuned 0.5 by the 0.45.2 re-tune, an even stronger anti-stall. Split out of ProtectValue |
