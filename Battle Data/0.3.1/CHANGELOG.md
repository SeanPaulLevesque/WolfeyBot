...
Two functions built `Combatant` objects for opponent Pokémon without checking
`opp.fainted`, allowing dead mons to participate in speed calculations.

**`_opp_combatant()` in `decision.py`:** `PrioritySpeedModule` calls this helper
to build the `opp_combatants` list it feeds into `will_outspeed()`.  With a
fainted mon present, `_speed_outcomes()` in `turn_order.py` looked up the dead
mon's species in the speed-distribution database.  If the species was unknown
(e.g. a Champions-format Mega that isn't in the database), `_build_dist` returned
an empty list, causing `_speed_outcomes` to fall back to the sentinel `(100, 1.0)`.
This produced a phantom 100-speed combatant that distorted the speed comparison,
and in some cases caused the entire `will_outspeed()` call chain to silently raise
an exception caught by the per-module `try/except`, leaving all actions at weight
1.0 with no scoring reasons logged.

**`build_combatants()` in `turn_order.py`:** The broader combatant builder used
by `estimate_turn_order` had the same gap — fainted mons were iterated and turned
into `Combatant` objects.

**Fix:** Added `or mon.fainted` to the guard in both locations.  `_opp_combatant`
now returns `None` for fainted slots (the downstream `if c is not None` filter in
`PrioritySpeedModule` already handles `None`).  `build_combatants` now skips
fainted slots with `continue`.

Root cause confirmed in two 0.3.0 battles:
- Battle 2616566089: Delphox-Mega (unknown species) active while terrain present →
  all-1.0 turns for Aerodactyl-Mega on turns 2, 3, 5.
- Battle 2616605299: Mamoswine at 0/0 HP still present in `opp_actives` →
  all-1.0 turn for the surviving slot.

### Tuning / Optimisations

#### ProtectModule — steeper multiplier at critical HP (`decision.py`)
The low-HP Protect bonus was a flat ×1.5 for any HP below 25%.  At extreme HP
values (sub-5%), attacks still outscored Protect because the damage modules
produce large multipliers even when the attacking move's expected value is low
relative to the real danger.

Examples from 0.3.0 data:
- Sneasler at 0.6% HP: Close Combat ×3.41 vs Protect ×3.0 — attacked, survived
  by luck.
- Kingambit at 5.8% HP: Kowtow Cleave ×3.23 vs Protect ×2.25 — attacked,
  fainted next turn.

**Fix:** Added a `CRITICAL_HP_THRESHOLD` (5%) with a separate `CRITICAL_HP_FACTOR`
(×3.0).  The two tiers are mutually exclusive (`elif`), so only the steeper
factor applies in the critical zone:

- HP < 5%:  Protect gets ×3.0 (replaces ×1.5)
- HP 5–25%: Protect gets ×1.5 (unchanged)
- HP ≥ 25%: no HP-based bonus (unchanged)

Both tiers stack on top of `THREATENED_FACTOR` (×2.5) as before, so a mon at
0.6% HP that faces any OHKO threat receives ×7.5 on Protect — well clear of any
plausible attack score.

#### DoublingUpModule — redirect when partner has a near-certain KO (`decision.py`)
Statistical review of 0.3.0 battles showed 37.2% of two-slot turns had both mons
targeting the same opponent.  The `_FACTORS` penalties (×0.55–0.85) were
insufficient to redirect Slot 1 when Slot 0 had acquired a large damage bonus
(×10+) from `DamageOutputModule` + `ThreatEliminationModule`.

When Slot 0 is very likely going to KO the target, Slot 1 attacking there too is
wasteful — the target will be gone, and the other opponent was left unchecked for
free.

**Fix:** After recording the normal doubling-up penalty, check whether the
partner's committed action weight meets `_PARTNER_KILLS_THRESHOLD` (≥10.0 —
consistent with a guaranteed or near-guaranteed OHKO).  If so, apply an
additional `PARTNER_KILLS_FACTOR` (×0.65) on top of the existing penalty.

Combined effective factors when partner kills:
- Target protected last turn + other not threatening:  ×0.85 × 0.65 ≈ ×0.55
- Target protected last turn + other threatening:      ×0.70 × 0.65 ≈ ×0.46
- Neither condition:                                   ×0.55 × 0.65 ≈ ×0.36

At the most common attack score range (×2–4), this is now enough to redirect Slot
1 to the other opponent even when that opponent has not been evaluated for damage.
